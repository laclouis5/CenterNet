...
class opts(object):
  def __init__(self):
    ...
    # Add option --not_reg_hw to only regress heatmaps
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'exdet':
      ...
      opt.heads = {'hm': opt.num_classes, 'wh': 2, 'hps': 34} # Obj hm | box size | keypoints location
      if opt.reg_offset:
        opt.heads.update({'reg': 2}) # Box offset
      if opt.hm_hp:
        opt.heads.update({'hm_hp': 17}) # Keypoint heatmaps
      if opt.reg_hp_offset:
        opt.heads.update({'hp_offset': 2}) # Keypoint offsets
    else:
